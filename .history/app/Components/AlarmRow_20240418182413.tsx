...
const AlarmRow: React.FC<AlarmRowProps> = ({ title, time, alarm, id }) => {
    ...
    return (
        <div className="flex text-xs row w-full justify-around text-[#A7A7A7] items-center">
            <p className="flex mx-auto">{title}</p>
            <p className="">{time}</p>
            <select id="alarm" defaultValue={alarm} value={alarmValue} onChange={changeAlarm} className="bg-[#35334D] rounded-md shadow-md h-10 px-2 w-fit focus:outline-none">
