...
const AlarmRow: React.FC<AlarmRowProps> = ({ title, time, alarm, id }) => {
    ...
    return (
        <div className="flex text-xs row w-full justify-center text-[#A7A7A7] items-center">
            <p className="flex w-fit">{title}</p>
            <p className="flex w-full">{time}</p>
            <select id="alarm" defaultValue={alarm} value={alarmValue} onChange={changeAlarm} className="flex w-full text-center bg-[#35334D] rounded-md shadow-md h-10 px-2 focus:outline-none">
