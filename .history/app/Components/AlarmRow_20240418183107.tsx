...
const AlarmRow: React.FC<AlarmRowProps> = ({ title, time, alarm, id }) => {
    ...
    return (
        <div className="flex lex-row text-xs sm:text-sm text-center w-full justify-around text-[#A7A7A7] items-center">
            <p className="flex w-fit justify-center text-center">{title}</p>
            <p className="flex w-fit text-center">{time}</p>
            <select id="alarm" defaultValue={alarm} value={alarmValue} onChange={changeAlarm} className="flex w-fit text-center bg-[#35334D] rounded-md shadow-md h-10 px-2 focus:outline-none">
