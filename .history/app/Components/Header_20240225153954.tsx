...
export default function Header() {
    return (
        <div className="flex h-24 w-full px-10 py-5 bg-[#1A1926] shadow justify-between">
            <Image src={logo} alt="Plan-It-Earth" height={60} width={60} className="h-35 w-35 mt-1 cursor-pointer hover:opacity-85"/>
            <div className="w-52 sm:hidden lg:block"></div>
            <Search />
            <div className="flex flex-row h-full items-center gap-20">
                <Image src={alarm} alt="Notifications" height={35} width={35} className="h-8 w-8 hover:opacity-85"/>
                <Image src={share} alt="Share" height={35} width={35} className="h-8 w-8 hover:opacity-85"/>
