...
export default function Alarm() {
    ...
    const formatDate = (date) => {
        ...
        return date.toLocalString('en-US', options).replace(',', ' @');
    }
    ...
    return (
        <div className="bg-[#16141C] h-lvh justify-center">
            ...
            <div className="flex flex-col mx-2 sm:mx-36 md:mx-36 lg:mx-36 py-8 bg-[#1A1926] mt-6 border shadow-lg border-white rounded-md">
                ...
                <div className="flex flex-col w-full gap-6 mt-6 px-4 justify-around">
                    {events.length > 0 ? (
                        <div className="flex flex-col gap-6 px-4">
                            {events.map(event => (
                                <AlarmRow key={event.id} title={event.title} time={formatDate(event.date)} alarm={event.extendedProps.alarm} id={event.id} />
                            ))}
                        </div>
                    ) : (
