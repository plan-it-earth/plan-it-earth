...
export default function CreateNote() {
    ...
    const handleTimeChange =(event) => {
        ...
        if (time.length > inputValue.length && time[time.length - 1] === ':') {
            inputValue = inputValue.slice(0, -1);  // Remove last character which is the colon
          }
      
          // Normalize the input to ensure it's only numbers or colon
          let numericInput = inputValue.replace(/:/g, ''); // Remove colon for processing
          if (numericInput.length > 4) {
            numericInput = numericInput.slice(0, 4); // Limit to four digits
          }
                ...
                <form onSubmit={handleSubmit} className="flex flex-col space-y-4">
                    <div>
                        ...
                        <div className="flex flex-row justify-between">
                            ...
                            <input type="text" id="time" name="time" required value={time} onChange={handleTimeChange} placeholder="12:00" className="text-white bg-gray-600 mt-1 px-3 py-2 rounded-md w-20 text-center focus:outline-none"/>
